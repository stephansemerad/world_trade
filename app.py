...
continent_selection = st.sidebar.multiselect(
    "Continent:", options=[name for name in continent_options]
)
...
if continent_selection:
    filtered_countries = countries[
        countries["continent_name"].isin(continent_selection)
    ]
else:
    filtered_countries = countries
...
country_options = [
    (name, iso3)
    for name, iso3 in zip(filtered_countries["name"], filtered_countries["iso_3"])
]
        ...
        chart = (
            ...
            .encode(
                x=alt.X(
                    ...
                    scale=alt.Scale(
                        padding=0.4
                    ),  # CORRECT: scale padding (0-1), not axis
                ),  # 1 decimal, e.g., 1.2M
...
with population_data:
        ...
        chart = (
            ...
            .encode(
                ...
                y=alt.Y(
                    "name:N",
                    sort=alt.EncodingSortField(
                        field="pct_growth", op="max", order="descending"
                    ),
                    title="Country",
                ),
                tooltip=[
                    alt.Tooltip("name:N"),
                    alt.Tooltip("pct_growth:Q", format=".1f"),
                ],
            )
        ...
        chart = (
            ...
            .encode(
                ...
                y=alt.Y(
                    "name:N",
                    sort=alt.EncodingSortField(
                        field="value", op="max", order="ascending"
                    ),
                    title="Country",
                ),
                tooltip=[alt.Tooltip("name:N"), alt.Tooltip("value:Q", format=".3s")],
        ...
        chart = (
            ...
            .encode(
                ...
                y=alt.Y(
                    "name:N",
                    sort=alt.EncodingSortField(
                        field="pct_growth", op="max", order="ascending"
                    ),
                    title="Country",
                ),
                tooltip=[
    ...
    with col2:
        chart = (
            ...
            .properties(
                title="Population Growth % over Time per Country", height=600, width=800
            )
            .interactive()
        )
        st.altair_chart(chart, use_container_width=True)
