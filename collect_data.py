...
countries_name = {
    x.iso_3: x.name
    for x in session.query(Country.iso_3, Country.name)
...
trades = [
    f"{x.year}-{x.product_id}-{x.reporter}-{x.partner}".lower()
    for x in session.query(Trade.year, Trade.product_id, Trade.reporter, Trade.partner)
    .distinct()
    .all()
]
        ...
        for reporter in countries_reporter:
            for partner in countries_partner:
                ...
                if reporter == partner:
                    ...
                    print(
                        slug,
                        countries_name[reporter.iso_3],
                        "->",
                        countries_name[partner.iso_3],
                    )
